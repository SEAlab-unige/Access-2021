...
def get_segmentation_array(image_input, nClasses, width, height, no_reshape=False):
    ...
    img = image_resize_no_dist(img, width=width, height=height, inter=cv2.INTER_NEAREST)
    img = img[:, :, 0]
    ...
    while True:
        ...
        for _ in range(batch_size):
            ...
            X.append(get_image_array(im, input_width,
                                     input_height, ordering="channel last"))

            Y.append(get_segmentation_array(
                seg, n_classes, output_width, output_height))

            # fake_Y = np.zeros((output_width*output_height,9))
            # fake_Y[:,8]=1
